...
@section('content')
    ...
    {{ Form::select('client_id',  App\Models\Client::pluck('name','id'), null, ['class'=>'form-control', 'id'=>'clientName']) }}
    ...
    {{ Form::text('chequeName', old('chequeName'), ['class'=>'form-control', 'id'=>'chequeName']) }}
    @if($errors->has('chequeName'))
        <span style="color:red;">{{ $errors->first('chequeName') }}</span>
    @endif

    {{ Form::label('Cheque Number: ') }}
    {{ Form::text('chequeNumber', old('chequeNumber'), ['class'=>'form-control', 'id'=>'chequeNumber']) }}
